...
class AdminHomestayEditProvider extends ChangeNotifier {
...
//Edit the details of a customer of rooms,dormitory,homestay.
  Future<void> UpdatehomestayData(
    BuildContext context, {
    required String oldHomestayName,
    required String NewHomestayName,
    ...
    try {
      ...
      QuerySnapshot querySnapshot =
          await Homestaycollection.where('name', isEqualTo: oldHomestayName)
              .get();
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        Homestaycollection
            ...
            // .then((value) {
            //   Provider.of<AdminHomestayProvider>(context, listen: false)
            //       .loadHomestays();
            //   if (kDebugMode) {
            //     print(
            //         'Homestay data updated on firestore , roomno $NewHomestayName');
            //   }
            //   notifyListeners();
            // })
            .then((value) => ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Homestay Data Edited'),
                    duration:
                        Duration(seconds: 2), // You can adjust the duration
                  ),
                ))
            .then((value) => Navigator.of(context).pop([
                  Provider.of<HomestayEditListProvider>(context, listen: false)
                      .DetailsLoader()
                ]));
      }
